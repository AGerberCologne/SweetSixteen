 ...
 * Die Serverengine enthält die "Spielmatrix". 
 * 
 * @author Alina Gerber, 5961246
 * 
 */
	 ...
	 *  Außerdem sendet sie periodisch die Monsterbwegungen.
	 *  
	 *  @author Alina Gerber, 5961246
	 */
	public void run() {
...
// Monster Methoden
	 ...
	 * Die Zulässigkeit wird mit der Methode zulaessig() abgefragt. Die Nachricht wird nur verschcikt, wenn m.nachricht = true ist, also der Schritt im Server auch wirklich gemacht wurde.
	 * @author Alina Gerber, 5961246
	 * @throws InterruptedException
	 */
...
// Spieler Methoden
	...
	/**
	 * Die Methode geht die beiden Listen der x-Position und y-Position durch und übergibt diese an den Client.
	 * Außerdem wird nach jedem Positionswechsel des Spielers die MonsterBewegung einmal aufgerufen
	 * @throws InterruptedException
	 * 
	 * @author Alina Gerber, 5961246
	 */
	...
	/**
	 * Dieser Methode wird das aktuell vom eingeloggten Spieler übergeben und dann dieses Level 
	...
	public void speichern(int levelNr) {
		...
		try {
			...
			FileReader fr = new FileReader("Spielerdaten");//Initialisierung des Filereaders für das Original
			BufferedReader br = new BufferedReader(fr);//Initialisierung des BufferedReaders für das Original
			FileWriter fw = new FileWriter("Spielerdaten2");//Initialisierung des Filewriters für die Zwischendatei
			BufferedWriter bw = new BufferedWriter(fw);//Initialisierung des BufferedWriters
			String zeile;//Zwischenspeicher für die gelesenen Strings vom BufferdReader
			while ((zeile = br.readLine()) != null) {//Während das Dokument noch nicht ganz durchgegangen ist, tue..
				if (zeile.equals(abgleich)) {//teste, ob der name und das passwort mit der gelesenen Zeile übereinstimmen
					bw.write(zeile);// Wenn diese übereinstimmen, dann schreibe die Zeile in die Hilfsdatei
					bw.newLine();//Gehe in die nächste Zeile in der Hilfsdatei eine Zeile weiter
					bw.write(neuesLevel);//Schreibe das neue Level in diese Zeile
					bw.newLine();//Gehe in die nächste Zeile in derHilfsdatei
					br.readLine();//Lese die nächste Zeile (=Zeile mit altem Level)
					zeile = br.readLine();//Lese die nächste Zeile des Originals
				}
